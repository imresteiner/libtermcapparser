 ...
 * fields to be.
 */
static inline int termchars_equal_override(termchar *a, termchar *b,
				    unsigned long bchr, unsigned long battr)
{
  /* FULL-TERMCHAR */
  if (a->chr != bchr)
    return FALSE;
  if (unlikely((a->attr ^ battr) &~ DATTR_MASK))
    return FALSE;
  while (unlikely(a->cc_next && b->cc_next)) {
      a += a->cc_next;
      b += b->cc_next;
      if (likely(a->chr != b->chr))
        return FALSE;
    }
  return (!a->cc_next && !b->cc_next); /* false IFF one cc-list ends, other does not */
}
...
void term_free(Terminal *term)
{
	...
	sfree(term->pre_bidi_cache[i].chars);
	sfree(term->post_bidi_cache[i].chars);
	sfree(term->pre_bidi_cache[i].forward);
        sfree(term->post_bidi_cache[i].forward);
        sfree(term->pre_bidi_cache[i].backward);
